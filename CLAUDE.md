...
- **`Js*` 6 ตัวถูก cache 900 วินาที และล้างไม่ได้เลย** — `getCachedClientAssetContent_` [Code.js]
  ...
  **ห้ามสรุปว่า "แก้ผิด" จนกว่าจะตัดข้อนี้ออกไปก่อน**
  ★ **`JsReports` เพิ่งเข้ากลุ่มนี้จริงๆ** ก่อนหน้านี้ไฟล์ใหญ่เกินเพดาน 100 KB (115,010 ไบต์)
  จึงเขียน cache ไม่ติดเลย = แก้แล้วเห็นผลทันทีมาตลอด **อย่าเอาประสบการณ์เดิมมาใช้ต่อ**
- **เขียน cache ก้อนใหญ่ต้องใช้ `putLargeCachedJsonByKey_`** ซึ่งหั่นตาม**ไบต์** (`LARGE_CACHE_CHUNK_MAX_BYTES`)
  เพดาน CacheService คือ 100 KB/คีย์ **นับเป็นไบต์** และไทย 1 ตัว = 3 ไบต์
  `putCachedJsonByKey_` ตัวเล็กไม่เช็คขนาดและกลืน error = เขียนไม่ติดแบบเงียบสนิท
