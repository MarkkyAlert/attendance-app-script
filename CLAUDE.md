...
- **เขียน cache ก้อนใหญ่ให้ใช้ `putLargeCachedJsonByKey_`** ซึ่งหั่นตาม**ไบต์** (`LARGE_CACHE_CHUNK_MAX_BYTES`)
  เพราะเอกสาร CacheService ระบุเพดาน 100 KB/คีย์ **นับเป็นไบต์** และไทย 1 ตัว = 3 ไบต์
  ⚠️ **แต่วัดบนของจริงแล้วเพดานนี้ไม่ถูกบังคับตามที่เอกสารบอก** — `JsReports` ขนาด 107,972 ไบต์
  เขียนด้วยคีย์เดียวแล้วอ่านกลับได้ครบ (`cache:client_modules_fit`) **ห้ามสรุปว่า cache พลาด
  เพราะขนาดเกิน จนกว่าจะวัด** · `putCachedJsonByKey_` ตัวเล็กกลืน error ทิ้ง ถ้าพลาดจะเงียบสนิท
...
## อัปเดตเอกสารเมื่อไหร่
**ต้องอัปเดตใน commit เดียวกับที่แก้โค้ด ไม่ใช่ทำทีหลัง**
- แก้พฤติกรรมระบบ / เพิ่มลบฟังก์ชันสำคัญ / เปลี่ยนโครงชีต → ไล่ดู `ARCHITECTURE.md` ว่าส่วนไหนกลายเป็นเท็จ แล้วแก้
- เลือกทางหนึ่งโดยมีทางอื่นที่ทิ้งไป → `DECISIONS.md` (**เพิ่มอย่างเดียว ห้ามลบของเก่า**)
- แก้ย่อหน้าเหตุผลใน `ARCHITECTURE.md` → เช็ค `DECISIONS.md` ว่าซ้ำกันไหม ถ้าซ้ำให้แก้ทั้งคู่
  **ห้ามลบข้างใดข้างหนึ่งทิ้งเพราะคิดว่าซ้ำ**
- เจอกฎที่ถ้าไม่รู้แล้วจะเขียนโค้ดผิด → `CLAUDE.md`
- ปิดงาน / เจองานใหม่ → `TODO.md` · เปลี่ยนขั้นตอนติดตั้งหรือส่งมอบ → `README.md`
- **`UAT.md` อ้างของภายนอกตัวเอง 6 ประเภท** แก้โค้ดที่กระทบประเภทไหน ให้แก้ `UAT.md` ใน commit เดียวกัน
  **ไม่มีอะไรเตือนถ้าเอกสารกลายเป็นเท็จ** — คนเดินรอบถัดไปจะเชื่อมันแล้วเสียรอบทิ้ง
  1. ชื่อปุ่ม ชื่อหน้า **ข้อความยืนยันผล** → grep ชื่อเดิม
  2. ตัวเลขค่าคงที่ (toast 2500/4500 · cache 900 วิ · session ครู 2 ชม. · PIN ล็อก 15 นาที · rate limit)
  3. ข้อมูลจาก seed (จำนวนนักเรียน · ตารางเคสพิเศษ)
  4. รายการหน้าและฟีเจอร์ที่ต้องทดสอบ
  5. รายการ **"ห้ามกด"** — เพิ่มปุ่มที่ทำลายข้อมูลถาวรต้องเติมเข้าไป
  6. รายการ **"ทำไม่ได้"** — ปลดล็อกแล้วต้องลบออก ไม่งั้นถูกข้ามตลอดไป
  ★ ทดสอบจบรอบ → อัปเดต `UAT.md` **ก่อนแก้บั๊ก** เอาเฉพาะสิ่งที่ทำให้รอบนั้นเสียเวลาเปล่า · บั๊กไป `TODO.md`

**ถ้าไม่แน่ใจว่ากระทบ `ARCHITECTURE.md` ไหม ให้บอกว่าไม่แน่ใจ อย่าเดาว่าไม่กระทบ**
