...
### ★ ฝั่ง client ทิ้ง session ที่หมดอายุตั้งแต่ตอนอ่านกลับ — ทั้งครูและผู้ปกครอง

`expires_at` ถูกส่งลง client และเก็บใน `sessionStorage` ทั้งสองฝั่ง
**ต้องเทียบกับ `Date.now()` ตอนอ่านกลับ ไม่ใช่รอให้ server ปฏิเสธ**
ไม่ทำแล้วอาการคือ **จอกำลังโหลดค้างหลายวินาที แล้วค่อยเด้งไปหน้าเข้าสู่ระบบ**

| หน้า | ตัวเช็ค |
|---|---|
| ครู | `isStoredTeacherAuthExpired_` [JavaScript.html] เรียกจาก `initializeTeacherAuth_` |
| ผู้ปกครอง | `loadStoredParentAuth_` [ParentView.html] |

**กติกา 3 ข้อของการเช็คฝั่ง client** (ทั้งสามข้อเป็นเหตุผลของโค้ดที่เขียนไว้แล้ว ห้ามลดรูป)

1. **ไม่มี `expires_at` = ยังไม่หมดอายุ ไม่ใช่หมดอายุ** — session ที่เก็บไว้จากโค้ดเวอร์ชันก่อน
   ไม่มีฟิลด์นี้ ถ้าตีเป็นหมดอายุ ครูทุกคนจะถูกเตะออกหนึ่งครั้งตอนอัปเดตโค้ดโดยไม่มีเหตุผล
2. **เผื่อนาฬิกาเครื่องเพี้ยน** (`TEACHER_AUTH_CLOCK_SKEW_MS`) เพราะเทียบกับนาฬิกาเครื่องผู้ใช้
   ไม่ใช่ของ server
3. **ใช้เฉพาะตอนอ่านกลับ ไม่เอาไปตัดสินทุก request** — ตัวตัดสินจริงคือ `AUTH_EXPIRED`
   จาก server ซึ่งเชื่อถือได้กว่านาฬิกาเครื่อง และ `handleTeacherSessionExpired_` รับไว้อยู่แล้ว
   ถ้าเอาไปกั้นทุก request แล้วนาฬิกาเพี้ยน ครูจะใช้งานไม่ได้เลยแทนที่จะแค่ล็อกอินใหม่ครั้งเดียว
...
1. **โค้ด client ถูก cache 15 นาทีโดยไม่มีทางล้าง** — `getCachedClientAssetContent_` ใน `Code.js`
   ...
   ★ เคยเข้าใจผิดว่า `JsReports` ใหญ่เกินเพดานจึงไม่เคยติด cache — **วัดแล้วไม่จริง มันติดมาตลอด**
   (ดูหัวข้อ "เพดาน 100 KB ไม่ถูกบังคับตามที่เอกสารบอก") ทั้ง 6 ตัวค้างได้ 15 นาทีเท่ากันหมด
...
เดิมค่านี้คือ 80,000 **ตัวอักษร** ซึ่งถ้าเป็นไทยหนาแน่น (3 ไบต์/ตัว) จะได้ chunk ถึง 240 KB
`splitStringByUtf8Bytes_` ไม่ตัดกลาง surrogate pair เพราะตัดกลางคู่แล้วต่อกลับจะได้อักขระเสีย
และ `JSON.parse` จะพังทั้งก้อน

### ★★ วัดแล้ว: เพดาน 100 KB **ไม่ถูกบังคับตามที่เอกสารบอก**

`cache:client_modules_fit` (19 ส.ค. 2569) เขียน `JsReports` ขนาด **107,972 ไบต์ ด้วยคีย์เดียว**
แล้วอ่านกลับได้ครบถ้วนตรงต้นฉบับ · `broken_with_single_key` เป็น **ลิสต์ว่าง** ทั้ง 6 โมดูล

| โมดูล | ตัวอักษร | ไบต์ | เกิน 100 KB | คีย์เดียวเขียนติด |
|---|---:|---:|:--:|:--:|
| `JsReports` | 86,790 | **107,972** | ★ ใช่ | ✅ ติด |
| `JsDashboard` | 74,607 | 92,807 | ไม่ | ✅ ติด |
| อีก 4 ตัว | 11K–57K | 11K–71K | ไม่ | ✅ ติด |

→ **ห้ามสรุปว่า "cache เขียนไม่ติดเพราะขนาดเกิน" จากการคำนวณขนาดอย่างเดียว ต้องวัดเสมอ**
ยังไม่รู้ว่าเพดานจริงอยู่ที่เท่าไร รู้แค่ว่า 107,972 ไบต์ยังผ่าน

การหั่นตามไบต์จึงเป็น**การกันไว้ล่วงหน้า ไม่ใช่การแก้บั๊กที่เกิดขึ้นจริง**
⚠️ มีข้อเสียที่ต้องรู้: หั่นเป็นหลาย chunk แล้ว **chunk เดียวถูก evict = พลาดทั้งก้อน**
ขณะที่คีย์เดียวหลุดหรืออยู่ครบเป็นหน่วยเดียว
...
3. **`attendance_date_buckets`** — 10,000 แถว ≈ 1.5 MB → ~19 chunk ต้องอยู่ครบถึงจะ hit
   **ยิ่งข้อมูลโตยิ่ง hit rate ต่ำ** archive ภาคเรียนคือทางระบายเดียว
   ⚠️ **ตั้งแต่ 17 ส.ค. 2569 หน้าเช็คชื่อพึ่งถังนี้ด้วย** ไม่ใช่แค่หน้ารายงาน
   (`getCachedRawAttendanceRecordsByDate_` เสิร์ฟจากถังแทนการสแกนต่อวัน — เร็วขึ้น 2-3 วินาที
   ต่อวันที่เปิด) **แลกมาด้วยการที่หน้าที่ครูใช้ทุกเช้าไปผูกกับ cache ก้อนที่ใหญ่ที่สุด**
   ถ้าวันหนึ่งข้อมูลโตจน chunk หลุดบ่อย อาการจะกลับมาที่หน้าเช็คชื่อก่อนหน้าอื่น
   → ทางถอย: `readAttendanceRecordsByDate_` ยังอยู่ครบ เปลี่ยนกลับได้ที่จุดเดียว
